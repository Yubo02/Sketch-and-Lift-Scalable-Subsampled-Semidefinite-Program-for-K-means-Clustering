...
idx_hat_1(j)=mp; % Assigned to the position of center

end

idx_hat_1; % The index of groups after kmeans_sdp & sketching.
timeElapsed_1 = toc;
...
error_1(i)=sum(idx_ori'~=idx_hat_1)/n;
...
err1_p=[AA(1,2:size(AA,2)); AA(3,2:size(AA,2))]; % Error rate for different p
tim1_p=[AA(2,2:size(AA,2)); AA(3,2:size(AA,2))]; % Time cost for different p 